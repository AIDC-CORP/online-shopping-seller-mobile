    ...
    <div className="bg-white p-4 rounded-lg shadow-sm space-y-3">
      ...
      {order.status === OrderStatus.New && (
        <div className="flex space-x-2 pt-2">
          <button className="flex-1 bg-red-100 text-red-600 py-2 rounded-md text-sm font-medium hover:bg-red-200 transition">Từ chối</button>
          <button className="flex-1 bg-emerald-500 text-white py-2 rounded-md text-sm font-medium hover:bg-emerald-600 transition">Xác nhận</button>
        </div>
      )}
    </div>
  );
...
const Orders: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      <div className="p-1 bg-gray-200/75 rounded-full flex items-center space-x-1">
          {tabs.map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`flex-1 px-2 py-1.5 text-sm font-medium whitespace-nowrap transition-all rounded-full ${
                activeTab === tab
